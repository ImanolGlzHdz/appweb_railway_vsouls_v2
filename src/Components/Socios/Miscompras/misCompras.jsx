...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const responsePendientes = await fetch(`https://apivsoulsapi8-production.up.railway.app/miscompras/pendientes/${idUsuarioGlobal}`);
      const dataPendientes = await responsePendientes.json();
      setCompras2(dataPendientes[0]);

      // Luego, obtén las compras enviadas
      const responseEnviadas = await fetch(`https://apivsoulsapi8-production.up.railway.app/miscompras/enviadas/${idUsuarioGlobal}`);
      const dataEnviadas = await responseEnviadas.json();
      setCompras3(dataEnviadas[0]);

      // Finalmente, obtén las compras entregadas
      const responseEntregadas = await fetch(`https://apivsoulsapi8-production.up.railway.app/miscompras/entregadas/${idUsuarioGlobal}`);
