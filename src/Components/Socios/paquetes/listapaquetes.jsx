...
const Paquetes = ({ paquetes: paquetesProp, agregarpaquetesalcarrito }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API}/paquetes/activos`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API}/productos/paq`);
        const data = await response.json();
                      ...
                      .map((prod, index) => (
                        <div className='card2 col-md-12' key={prod.CLAVE_P}>
                          <a onClick={() => seleccionarProducto(prod)}>
                            <div className=''>
                              <img class='img-box' src={`${import.meta.env.VITE_API}/products/` + prod.CLAVE_P + '-IMG_PRINCIPAL.png'} alt="" />
                            </div>
                          </a>
