...
const ModalComponent = ({ showModal, handleCloseModal }) => {
 ...
 const finalizarVenta_bd = async () => {
    try {
      const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/finalizarventa/${idUsuarioGlobal}/${statusventa}/${totalfinal}`, {
        method: 'POST',
      });
      // alert(`https://apivsoulsapi8-production.up.railway.app/carrito/finalizarventa/${idUsuarioGlobal}/${statusventa}`)
      if (response.ok) {
...
const fetchData = async () => {
  try {
    const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/paquete/nombre/${idUsuarioGlobal}`);
...
.concat(carritoProductospv.map(item => item.NOMBRE_P));
    ...
    const response = await axios.post("https://apivsoulsapi8-production.up.railway.app/mercadopago/create_preference/socios", {
  ...
  const sumarProducto = async (idprod) => {
    try {
      const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/sumprod/${idUsuarioGlobal}/${idprod}`, {
        method: 'POST',
      });
  ...
  const restarProducto = async (idprod) => {
    try {
      const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/resprod/${idUsuarioGlobal}/${idprod}`, {
        method: 'POST',
      });
  ...
  const eliminarProducto = async (idprod) => {
    try {
      const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/elimprod/${idUsuarioGlobal}/${idprod}`, {
        method: 'POST',
      });
  ...
  const cargarTotales = async () => {
    try {
      if (idUsuarioGlobal) {
        ...
        const totalProd = await fetchData(`https://apivsoulsapi8-production.up.railway.app/carrito/totalpro/${idUsuarioGlobal}`);
        const totalPaqu = await fetchData(`https://apivsoulsapi8-production.up.railway.app/carrito/totalpaq/${idUsuarioGlobal}`);
        const total = await fetchData(`https://apivsoulsapi8-production.up.railway.app/carrito/total/${idUsuarioGlobal}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/monedas/cli/${idUsuarioGlobal}`);
        const data = await response.json();
  ...
  const cargarPaquetes = async () => {
    try {
      const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/paquetes/${idUsuarioGlobal}`);
      const data = await response.json();
  ...
  const sumarPaquete = async (idUsuario, idPaquete) => {
    try {
      await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/sumpaqu/${idUsuario}/${idPaquete}`, {
        method: 'POST',
      });
      cargarTotales();
      fetchCarritoData();
  ...
  const restarPaquete = async (idUsuario, idPaquete) => {
    try {
      await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/respaqu/${idUsuario}/${idPaquete}`, {
        method: 'POST',
      });
      cargarTotales();
      fetchCarritoData();
  ...
  const eliminarPaquete = async (idUsuario, idPaquete) => {
    try {
      await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/relimpaqu/${idUsuario}/${idPaquete}`, {
        method: 'POST',
      });
      cargarTotales();
      fetchCarritoData();
  ...
  const eliminarProductoPV = async (idprod) => {
    try {
      const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/elimprodpv/${idUsuarioGlobal}/${idprod}`, {
        method: 'POST',
      });
  ...
  const cargarProductosModalpv = async () => {
    if (idUsuarioGlobal) {
    try {
      const carritoResponse = await fetch(`https://apivsoulsapi8-production.up.railway.app/carrito/prev/${idUsuarioGlobal}`);
  ...
  <div className="carrito-items">
    {carritoProductospv.map((carritoprod, index) => (
      <div className="carrito-item" key={index}>
        <img width="80px" src={'https://apivsoulsapi8-production.up.railway.app/products/'+ carritoprod.CLAVE_PRODUCTO + '-IMG_PRINCIPAL.png'} alt="" />
