...
const Listapaquetes = () => {
  ...
  const cambiarstatus = async (idPaquete, status) => {
    ...
    fetch(`https://apivsoulsapi8-production.up.railway.app/paquetes/estatus/${idPaquete}/${statusValue}`, {
      method: "POST"
    })
      .then(respuesta => respuesta.json())
      .then(data => {
  ...
  const funcionEliminar = (id) => {
    fetch(`https://apivsoulsapi8-production.up.railway.app/paquetes/${id}`, {
      method: "DELETE"
    })
      .then(() => {
