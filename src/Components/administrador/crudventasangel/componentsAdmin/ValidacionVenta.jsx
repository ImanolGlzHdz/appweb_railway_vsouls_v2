...
import "./ValidacionVenta.css"
import logo from './logoVsouls.png';
...
const ValidacionVenta = () => {
  ...
  const handleClickGeneratePDF = (venta) => {
    if (venta.numeroVenta !== undefined) {
      generatePDF(venta);
      axios.put(`https://apivsoulsapi8-production.up.railway.app/validar-venta/${venta.numeroVenta}`)
        .then((response) => {
        })
        .catch((error) => {
  ...
  const obtenerVentasPendientes = () => {
    axios.get('https://apivsoulsapi8-production.up.railway.app/ventas-pendientes')
      .then((response) => {
        ...
        <table id='tablaValidacion' className='table table-hover'>
          ...
          <tbody>
            {datosVenta.map((venta, index) => (
              <tr key={index}>
                <td className='moveratras' >{venta.ID_VENTA}</td>
