...
const Pdfdescargable = () => {
  ...
  const sendHandler = () => {
    ...
    fetch('https://apivsoulsapi8-production.up.railway.app/pdf/post', {
      method: 'POST',
      body: formData,
    })
      .then((res) => res.text())
  ...
  const peticionapi = () =>{
    ...
    const funcionEliminar = (id) => {
      ...
      fetch(`https://apivsoulsapi8-production.up.railway.app/pdf/delete/${id}`, {
            ...
            <tbody className='moveratras'>
              {Pdf.map(publicidad => (
                <tr key={publicidad}>
                  ...
                  <object data={`https://apivsoulsapi8-production.up.railway.app/pdf/` + publicidad} type="application/pdf" width="100%" height="600px">
                      <p>El archivo PDF no puede ser mostrado. Puedes <a href={`https://apivsoulsapi8-production.up.railway.app/pdf/` + publicidad}>descargarlo aquí</a>.</p>
                  </object>
                      </td>
                  <td className='moveratras'>
