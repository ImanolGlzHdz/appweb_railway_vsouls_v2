...
const InicioPaquetes = () => {
  ...
  const agregarProductoAlPaquete = (idProducto) => {
    ...
    if (!productoExistente) {
      ...
      const nuevosProductos = productosAgregados.map(producto =>
        producto.id_producto === idProducto ? { ...producto, cantidad: cantidades[idProducto] + 1 } : producto
      );
  ...
  const agregarPaquete = () => {
    ...
    fetch("https://apivsoulsapi8-production.up.railway.app/paquetes", {
      method: 'POST',
      headers: {
    ...
    <div className='todopublicidad todobodypublicidad moveratras'>
      ...
      <table className="table table-dark table-hover moveratras">
        ...
        <tbody className='moveratras'>
          {Array.isArray(productos) && productos.map(({ claveproducto, nombreproducto, precio }) => (
            <tr key={claveproducto} className='moveratras'>
              <td className='moveratras'>{claveproducto}</td>
              <td className='moveratras'>{nombreproducto}</td>
              <td className='moveratras'>{precio}</td>
              <td className='moveratras'>
                <button className='btn btn-primary margen' onClick={() => agregarProductoAlPaquete(claveproducto)}>
                  <img width="30" height="30" src="https://img.icons8.com/color/48/add-product.png" alt="add-product" />
                </button>
              </td>
            </tr>
          ))}
      ...
      <table className="table table-dark table-hover moveratras">
        ...
        <tbody>
          {Array.isArray(productosAgregados) && productosAgregados.map(({ id_producto, claveproducto, nombreproducto, precio, cantidad }) => (
            <tr key={id_producto}>
              <td>{claveproducto}</td>
              ...
              <td>
                <button className='btn btn-danger margen' onClick={() => restarpaquete(id_producto)}>
                  <img width="24" height="24" src="https://img.icons8.com/ios-glyphs/30/minus-math.png" alt="minus-math"/> 
                </button>
                <button className='btn btn-primary margen' onClick={() => sumarpaquete(id_producto)}>
                  <img width="24" height="24" src="https://img.icons8.com/material-rounded/24/plus-math--v1.png" alt="plus-math--v1"/>
                </button>
                <button className='btn btn-danger margen' onClick={() => eliminarProductoDelPaquete(id_producto)}>
                  <img width="30" height="30" src="https://img.icons8.com/ios/50/delete--v1.png" alt="delete--v1" />
                </button>
              </td>
            </tr>
          ))}
