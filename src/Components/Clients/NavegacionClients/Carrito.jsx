...
const Carrito = () => {
  ...
  const createPreference = async () => {
    try {
      ...
      const response = await axios.post("https://apivsoulsapi8-production.up.railway.app/mercadopago/create_preference", {
  ...
  const updateCantidadEnBaseDeDatos = async () => {
    for (const itemId in cantidades) {
      const userId = sessionStorage.getItem('userId');
      const url = `https://apivsoulsapi8-production.up.railway.app/actualizar-canti/${userId}/${itemId}/${cantidades[itemId]}`;
      try {
  ...
  const eliminarDelCarro = (productoClave) => {
    const userId = sessionStorage.getItem('userId');
    const url = `https://apivsoulsapi8-production.up.railway.app/borrar-producto-carrito/${userId}/${productoClave}`;

    axios.delete(url)
      .then(response => {
        alert('Borrado del carrito exitosamente');
        const userId = sessionStorage.getItem('userId');
        axios.get(`https://apivsoulsapi8-production.up.railway.app/ver-carrito/${userId}`)
          .then(response => {
            const cantidadesIniciales = {};
            response.data[0].forEach(item => {
                ...
                {carritoData.map(item => (
                  <tr key={item.CLAVEPRODUCTO}>
                    <th scope="row" className='imgTable'>
                      <img src={'https://apivsoulsapi8-production.up.railway.app/products/' + item.CLAVEPRODUCTO + '-IMG_PRINCIPAL.png'} style={{ width: "100%" }} alt="" />
                    </th>
