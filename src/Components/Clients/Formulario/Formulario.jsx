...
function Formulario (){
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://apivsoulsapi8-production.up.railway.app/vUsuario", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        if (id != undefined){
            ...
            try {
              const response = await fetch(`https://apivsoulsapi8-production.up.railway.app/rolUsuario/${id}`);
              const data = await response.json();
