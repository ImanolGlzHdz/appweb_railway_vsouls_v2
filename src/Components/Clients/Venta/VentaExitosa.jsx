...
const VentaExitosa = () => {
  ...
  const fetchVentaExitosaData = async () => {
    try {
      ...
      const url = `https://apivsoulsapi8-production.up.railway.app/venta-exit/${userId}`;
  ...
  const borrarCarroPorUsuario = () => {
    const userId = sessionStorage.getItem('userId');
    const url = `https://apivsoulsapi8-production.up.railway.app/borrar-carro/${userId}`;
  ...
  const insertarVenta = async (userId, total) => {
    try {
      ...
      if (ID_CLIENTE === "undefined") {
          const url = `https://apivsoulsapi8-production.up.railway.app/insertar-ventaSnC/${userId}/${total}`;
          ...
          const url = `https://apivsoulsapi8-production.up.railway.app/insertar-venta/${userId}/${total}/${ID_CLIENTE}`;
          const response = await axios.post(url);
          console.log('Respuesta del servidor (insertarVenta):', response.data);
